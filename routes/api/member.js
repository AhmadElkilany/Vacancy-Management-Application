             ...
             for(const vac of allvacancies){//check availability,location and skills
                 if(vac.status==="available" && vac.location===currentMember.placeOfResidence){
                     //for( const skill of vac.requiredSkills){
                         const skillaya = vac.requiredSkills
                         const skillDivided = skillaya.split(" ")
                         for(const x of skillDivided){
                             const curMemSkills = currentMember.skills.split(" ")
                             if(curMemSkills.includes(x) && !(recommendedJobs.includes(vac)))
