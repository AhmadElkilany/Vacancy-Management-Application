...
class TableRowVacancyMember extends Component {
  constructor(props) {
    super(props);
  ...
  apply() {
    axios
      .put(
        "http://localhost:4000/api/vacancy/apply/5ca0c819f792812168c302e0/" +
          this.props.obj._id
      )
      .then(console.log("Applied"))
      .then((window.parent.location = window.parent.location.href))
      .then(alert("You have applied for this vacancy sucessfully!"))
  ...
  render() {
    return (
      <tr>
        ...
        <td>
          <button onClick={this.apply} className="btn btn-danger">
            Apply
          </button>
        </td>
      </tr>
...
export default TableRowVacancyMember;
